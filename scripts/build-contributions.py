...
def card(pr):
    return f"""          <a class="oss-card" href="{escape(pr['url'], quote=True)}" target="_blank" rel="noopener">
            <div class="oss-card-top">
              ...
              <span class="oss-badge {pr['state']}">{pr['state'].capitalize()}</span>
            </div>
...
def main():
    ...
    print(f"{len(merged)} merged, {len(opened)} open, {len(repos)} repos — {stamp}")
...
def selfcheck():
    ...
    # PR titles are attacker-controlled text landing in HTML — must not break out.
